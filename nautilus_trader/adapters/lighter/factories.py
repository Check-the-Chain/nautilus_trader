...
def _resolve_environment(
    environment: LighterEnvironment | None,
    testnet: bool,
) -> LighterEnvironment:
    if environment is not None:
        return environment
    return LighterEnvironment.TESTNET if testnet else LighterEnvironment.MAINNET


def _resolve_proxy_url(config: LighterDataClientConfig | LighterExecClientConfig) -> str | None:
    return config.proxy_url or config.http_proxy_url or config.ws_proxy_url
...
def get_cached_lighter_http_client(
    *,
    ...
    return nautilus_pyo3.LighterHttpClient(  # type: ignore[attr-defined]
        ...
        is_testnet=environment == LighterEnvironment.TESTNET,
        proxy_url=proxy_url,
