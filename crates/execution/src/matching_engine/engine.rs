...
use nautilus_model::{
    ...
    enums::{
        ...
        OrderSide, OrderSideSpecified, OrderStatus, OrderType, PositionSide, PriceType, RecordFlag,
...
impl OrderMatchingEngine {
    ...
    fn cap_queue_positions_to_book(&mut self) {
        ...
        let keys: Vec<ClientOrderId> = self.queue_ahead_total.keys().copied().collect();
        for client_order_id in keys {
            let Some(&(price_raw, ahead_raw)) = self.queue_ahead_total.get(&client_order_id) else {
                continue;
            };
            let Some(side) = self
                .cache
            ...
            let capped_ahead_raw = ahead_raw.min(displayed.raw);
            if capped_ahead_raw < ahead_raw {
                self.reduce_queue_ahead(client_order_id, price_raw, ahead_raw, capped_ahead_raw);
            }
        }
    }
