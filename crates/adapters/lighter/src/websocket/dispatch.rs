...
///
/// Callers translate the parsed report through [`WsDispatchState`] so order
/// identity is validated against both client and venue order IDs.
...
pub(crate) async fn lookup_order_status_report(
    ...
    let finalize =
        |order: &LighterOrder, context: HttpOrderReportContext| -> Option<OrderStatusReport> {
            let report = parse_http_order_to_report(order, registry, account_id, ts_init, context)?;
