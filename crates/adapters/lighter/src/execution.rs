...
pub struct LighterExecutionClient {
    ...
    recent_order_states: Arc<Mutex<AHashMap<String, (OrderStatus, UnixNanos, String)>>>,
...
impl LighterExecutionClient {
    ...
    fn resolve_venue_order_id(
        ...
        venue_order_id
            .or_else(|| self.core.cache().venue_order_id(client_order_id).cloned())
            .or_else(|| {
...
impl ExecutionClient for LighterExecutionClient {
    ...
    fn batch_cancel_orders(&self, cmd: &BatchCancelOrders) -> anyhow::Result<()> {
        let mut cancels = Vec::new();
        for cancel in cmd.cancels.clone() {
            let venue_order_id =
